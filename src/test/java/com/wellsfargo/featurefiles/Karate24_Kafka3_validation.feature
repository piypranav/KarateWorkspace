...
Feature: Kafka Test Suite 24
  ...
  Scenario: Creating Consumer
    ...
    Then status 200
    * print response
    * match response.instance_id == "piyush_consumer"
  ...
  Scenario: Subscribing to topic
    Given path "/consumers/my-consumer-group/instances/piyush_consumer/subscription"
    And request
      """
      {
        "topics" : [ "piyush_topic" ]
  ...
  Scenario: Getting Subscription
    ...
    Then status 200
    * print response
    * assert response.topics[0] == "piyush_topic"


  Scenario: Getting Subscription records
    Given path "/consumers/my-consumer-group/instances/piyush_consumer/records"
  ...
  Scenario: Getting assigned Assignments
    Given path "/consumers/my-consumer-group/instances/piyush_consumer/assignments"
    And header Accept = "application/vnd.kafka.v2+json"
    When method get
    Then status 200
    * print response
    * assert response.partitions[0].topic == "piyush_topic"
  ...
  Scenario: Deleting consumer
    Given path "/consumers/my-consumer-group/instances/piyush_consumer"
    And header Accept = "application/vnd.kafka.v2+json"
    When method delete
    Then status 204
